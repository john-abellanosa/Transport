...
<head>
    <meta charset="UTF-8">
    <title>@yield('title', 'Company Login')</title>
...
<body>
    ...
    <div class="container" id="loginContainer">
        <div class="left">
            <img src="{{ asset('img/company.png') }}" alt="Company">
        </div>
        ...
        <div class="right-section">
            <div class="right-side">
                ...
                <div class="login-form">
                    <h1 class="welcome-title">Welcome to Company Portal</h1>
                    ...
                    <form id="loginForm" method="POST" action="{{ route('company.authenticate') }}">
                        @csrf
                        ...
                        <div class="form-options">
                            ...
                            <a href="{{ route('company.password.forgot') }}" class="forgot-password">Forgot
                                password?</a>
                        </div>
    ...
    <script> 
        @if(session('lockout_until'))
            ...
            localStorage.setItem('company_lockout_until', serverLockoutUntil);
        @endif
    </script>

        <script>
        @if ($errors->has('username'))
            document.getElementById("usernameError").textContent = "{{ $errors->first('username') }}";
            document.getElementById("usernameError").style.display = "block";
            document.getElementById("username").classList.add("error");
        @endif

        @if ($errors->has('password'))
            document.getElementById("passwordError").textContent = "{{ $errors->first('password') }}";
...
@vite(['resources/js/company/login/validation.js'])
